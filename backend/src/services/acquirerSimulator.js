...
async function processPayment(acquirerId, transaction) {
  ...
  if (special === 'ALWAYS_SUCCESS') {
    ...
    const effectiveRate = acq.anomalyMode ? Math.min(acq.currentSuccessRate, 0.15) : acq.currentSuccessRate;
  ...
  if (acq.totalTransactions > 0) {
    acq.currentSuccessRate = acq.successfulTransactions / acq.totalTransactions;
  }
